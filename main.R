...
for (ii in 1:nsim) {
  for (jj in 1:k_max) {
    y1 = dgp1(T,0.7,0)
    y2 = dgp2(T,0.6,0)
    y1_effective = y1[(k_max+1):T,1]
    y2_effective = y2[(k_max+1):T,1]
    resi1 <- correct_fit(y1,jj,k_max)
    resi2 <- correct_fit(y2,jj,k_max)

    aic_criterion_values_y1[ii,jj] = log(t(resi1)%*%resi1/(T-2*k_max-jj)) + 2*jj/(T-2*k_max-jj)
    bic_criterion_values_y1[ii,jj] = log(t(resi1)%*%resi1/(T-2*k_max-jj)) + jj*log(T-2*k_max-jj)/(T-2*k_max-jj)
    crv_criterion_values_y1[ii,jj] = cv_hv(y1,jj,10,20)

    aic_criterion_values_y2[ii,jj] = log(t(resi2)%*%resi2/(T-2*k_max-jj)) + 2*jj/(T-2*k_max-jj)
    bic_criterion_values_y2[ii,jj] = log(t(resi2)%*%resi2/(T-2*k_max-jj)) + jj*log(T-2*k_max-jj)/(T-2*k_max-jj)
